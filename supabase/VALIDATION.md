...
| A _new_ booking attempt for Tomáš       | `BIRTH_YEAR_OUT_OF_RANGE`                      |
...
`v_events := v_events || 'SESSION_SCHEDULE_CHANGED'` raises _malformed array
literal_: with no cast, Postgres parses the untyped literal on the right of `||`
...
### The first run against a fresh stack never sees any of it

Worth stating plainly, because for a while this section claimed coverage the
suite was not delivering.

Against a **freshly started** Supabase CLI stack, Realtime does not route
changes to an RLS-scoped subscriber until the integration suite has run once.
Everything else passes; these checks report:

```
SKIP  Realtime checks — the local stack is not routing changes yet.
```

A skip leaves the exit code at zero. CI starts a fresh stack on every run, so
CI's run was always the first one — which meant **these checks had never
actually run in CI**, and the suite reported success regardless. The first CI
run on the first pull request is what surfaced it, in the logs of a job that
had just passed.

The cause is still not understood, and the effort spent on it is recorded
honestly: a `service_role` subscriber on the same table routes immediately
after a restart, restarting the Realtime container by hand does not help, and
the only thing that reliably does is having run the suite once. That points at
state the CLI's Realtime tenant builds on first use rather than at anything in
Trainlio, but pointing is not knowing.

**So CI runs the suite twice**, and the second run sets `REALTIME_REQUIRED=1`,
which turns the skip into a failure. Running twice without that flag would
prove nothing: two skips look exactly like coverage. The warm-up may skip; the
run that counts may not.

To reproduce all three states:

```bash
pnpm db:reset
REALTIME_REQUIRED=1 pnpm db:integration   # fails: the flag can genuinely fail

pnpm db:reset
pnpm db:integration                       # skips, exit 0
REALTIME_REQUIRED=1 pnpm db:integration   # passes the Realtime checks
```

The first command matters most. Without it, a pass on the third proves only
that nothing threw.
