...
select pg_temp.expect_error($$insert into public.training_sessions(workspace_id,sport_id,location_id,facility_id,main_coach_profile_id,start_at,end_at,created_by)
  ...
  'facility must belong to the stated location (AC-130)');
select pg_temp.expect_error($$insert into public.training_sessions(workspace_id,sport_id,location_id,facility_id,main_coach_profile_id,start_at,end_at,created_by)
...
select pg_temp.expect_error($$insert into public.workspace_athlete_memberships(workspace_id,athlete_id,athlete_sport_profile_id,sport_id)
  ...
  'membership sport must match the workspace sport (AC-133)');
...
select pg_temp.expect_error($$update public.bookings set cancelled_at=now() where athlete_id='00000000-0000-0000-0000-0000000a0001'$$,
  'CONFIRMED booking cannot carry cancellation columns (AC-134)');
...
select pg_temp.expect_error($$delete from public.athletes where id='00000000-0000-0000-0000-0000000a0001'$$,
  'athlete with history cannot be deleted (AC-135)');
select pg_temp.expect_error($$delete from public.training_sessions where id='00000000-0000-0000-0000-0000000e0001'$$,
  'session with bookings cannot be deleted (AC-135)');
select pg_temp.expect_eq((select count(*)::text from public.audit_log), '1', 'audit row exists for the next two cases');
select pg_temp.expect_error($$update public.audit_log set action='SESSION_UPDATED'$$, 'audit_log rejects UPDATE (AC-141)');
select pg_temp.expect_error($$delete from public.audit_log$$, 'audit_log rejects DELETE (AC-141)');
...
select pg_temp.expect_eq(
  (select case when (start_at - interval '12 hours 1 second') <= start_at - make_interval(hours=>12) then 'ALLOWED' else 'BLOCKED' end
   from public.training_sessions where id='00000000-0000-0000-0000-0000000e0001'), 'ALLOWED', '12:00:01 before start → allowed (AC-040a)');
select pg_temp.expect_eq(
  (select case when (start_at - interval '12 hours') <= start_at - make_interval(hours=>12) then 'ALLOWED' else 'BLOCKED' end
   from public.training_sessions where id='00000000-0000-0000-0000-0000000e0001'), 'ALLOWED', '12:00:00 before start → allowed (AC-040b)');
select pg_temp.expect_eq(
  (select case when (start_at - interval '11 hours 59 minutes 59 seconds') <= start_at - make_interval(hours=>12) then 'ALLOWED' else 'BLOCKED' end
   from public.training_sessions where id='00000000-0000-0000-0000-0000000e0001'), 'BLOCKED', '11:59:59 before start → blocked (AC-040c)');
...
select pg_temp.expect_eq(
  ...
  'BIRTH_YEAR_OUT_OF_RANGE', 'new bookings use the narrowed rule (AC-172)');
...
select pg_temp.expect_eq(public.athlete_eligibility_for_session('00000000-0000-0000-0000-0000000e0001','00000000-0000-0000-0000-0000000a0001'),'ATHLETE_INACTIVE','new booking blocked (AC-181)');
...
select pg_temp.expect_eq((select status::text from public.bookings where training_session_id='00000000-0000-0000-0000-0000000e0001' and athlete_id='00000000-0000-0000-0000-0000000a0001'),'CANCELLED_BY_USER','guardian can still cancel (AC-182)');
...
select pg_temp.expect_eq((select count(*)::text from public.bookings where training_session_id='00000000-0000-0000-0000-0000000e0001'),'2','bookings preserved as historical evidence (AC-163)');

\echo ''
\echo '── D-18  history survives auth.users deletion ──────────────────────'
delete from auth.users where id='00000000-0000-0000-0000-0000000fa000';
select pg_temp.expect_eq((select (auth_user_id is null)::text from public.app_profiles where display_name='Rodina A'),'true','login link severed (AC-220)');
select pg_temp.expect_eq((select count(*)::text from public.bookings b join public.app_profiles p on p.id=b.created_by where p.display_name='Rodina A'),'2','bookings still attributed (AC-220)');
select pg_temp.expect_eq((select count(*)::text from public.guardian_athlete_access g join public.app_profiles p on p.id=g.profile_id where p.display_name='Rodina A'),'2','guardian links intact (AC-221)');
